...
class MediaCenter(QWidget):
    def __init__(self):
        ...
        self.zoom_scale = 1.0
        self.offset_x = 0      # смещение по оси X для pixmap
        self.offset_y = 0      # смещение по оси Y для pixmap
        self.last_mouse_position = QPoint()

        self.top_panel = QWidget(self)
        self.top_panel.setFixedHeight(55)
        self.top_panel.setStyleSheet("background-color: #002387;")
        ...
        self.bottom_panel.setStyleSheet("background-color: #002387;")
        layout.addWidget(self.bottom_panel)

        # Настройка кнопок и ползунка
        self.setupPanelWidgets()
    
    def setupPanelWidgets(self):
        # Кнопки на верхней панели
        top_layout = QHBoxLayout(self.top_panel)
        self.home_button = QPushButton("Home", self.top_panel)
        self.home_button.setStyleSheet("color: white;")
        top_layout.addWidget(self.home_button)

        # Ползунок на нижней панели
        bottom_layout = QHBoxLayout(self.bottom_panel)
        self.positionSlider = QSlider(Qt.Horizontal, self.bottom_panel)
        bottom_layout.addWidget(self.positionSlider)
    ...
    def updatePixmap(self):
        if self.label.isVisible():
            pixmap = QPixmap(self.folderMedia[self.currentMedia])
            scaled_pixmap = pixmap.scaled(
                int(self.display_width * self.zoom_scale),
                int((self.display_height - 160) * self.zoom_scale),
                Qt.KeepAspectRatio,
                Qt.SmoothTransformation
            )
            # Устанавливаем масштабируемое изображение в QLabel
            self.label.setPixmap(scaled_pixmap)
            self.label.setGeometry(self.offset_x, self.offset_y, scaled_pixmap.width(), scaled_pixmap.height())
            self.label.repaint()  # Обновляем label для перерисовки
    ...
    def paintEvent(self, event):
        if hasattr(self, 'label') and self.label.isVisible():
            ...
            if pixmap:
                target_rect = QRect(self.offset_x, self.offset_y, pixmap.width(), pixmap.height())
                painter.drawPixmap(target_rect, pixmap)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self.label.isVisible():
            self.dragging = True
            self.last_mouse_position = event.pos() - QPoint(self.offset_x, self.offset_y)

    def mouseMoveEvent(self, event):
        if self.dragging:
            self.offset_x = event.pos().x() - self.last_mouse_position.x()
            self.offset_y = event.pos().y() - self.last_mouse_position.y()
            self.updatePixmap()  # Обновляем изображение с новым смещением
